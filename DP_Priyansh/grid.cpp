...
// using dp, we are calculating path for every cell just once
// so, TC: O(n * m), SC: O(n * m)
ll f(ll i, ll j) {
	if (i >= n or j >= m) return INT_MAX;
	if (i == n - 1 and j == m - 1) return grid[n - 1][m - 1];

	if (dp[i][j] != -1) {
		// this state has been calculated before
		return dp[i][j];
	}
