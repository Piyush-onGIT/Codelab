...
void solve() {
    ...
    loop(i, 0, n) {
    	if (dpMin.find(a[i]) == dpMin.end()) dpMin[a[i]] = i + 1;
    }
    loop(i, 0, n) {
    	dpMax[a[i]] = i + 1;
    }
    ...
    while (q--) {
    	ll a, b;
    	cin >> a >> b;
    	if (dpMin.find(a) == dpMin.end() or dpMin.find(b) == dpMin.end()) cout << "NO\n";
    	else if (dpMin[a] <= dpMax[b] or dpMin[a] <= dpMin[b]) cout << "YES\n";
    	else cout << "NO\n";
    }
}
