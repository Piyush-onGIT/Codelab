...
#define dsort(x)                            sort(all(x)); reverse(all(x));
...
void solve() {
    ...
    cin >> n;
    ll spend[n], bud[n];
    loop(i, 0, n) cin >> spend[i];
    loop(i, 0, n) cin >> bud[i];

    vll groups;
    
    loop(i, 0, n) groups.pb(bud[i] - spend[i]);
    dsort(groups);

    ll e = n - 1, ans = 0;
    loop(i, 0, n) {
        while (e > i and groups[i] + groups[e] < 0) e--;
        if (e <= i) break;
        else {
            ans++;
            e--;
